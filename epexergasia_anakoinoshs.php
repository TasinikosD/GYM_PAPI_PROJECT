...
session_start(); // Αρχή session
    ...
    //ανάκτηση cookie και άντληση στοιχείων από τη βάση
    $k=$_COOKIE['anakid'];
    $stmt=$conn->prepare("SELECT * FROM ανακοινωσεισ where ID_ΑΝΑΚΟΙΝΩΣΗΣ=$k");
    $stmt->execute();
...
<html lang="en">
  ...
  <head>
    ...
    <!-- φόρμα για την αλλαγή των στοιχείων της ανακοίνωσης με εμφάνιση των ήδη υπάρχοντων -->
    <div class="main-banner" id="top">
        ...
        <div class="video-overlay header-text">
            ...
            <form action="" method="POST">
                ...
                <p class="forma">Τίτλος: <input type="text" value="<?= $row["ΤΙΤΛΟΣ"]?>" name="title" required></p><br>
                <p class="forma">Κείμενο:</p>
                <textarea name="content" rows="15" cols="40" required><?= $row["ΠΕΡΙΕΧΟΜΕΝΟ"]?></textarea><br>
                <input name="Accept" type="submit" value='ΑΠΟΘΗΚΕΥΣΗ'>
                <?php }?>
            </form>
            <?php
                ...
                if($title!=" "){
                    foreach($result as $row){
                        try{
                            $sql = "UPDATE ανακοινωσεισ SET ΤΙΤΛΟΣ = :title, ΠΕΡΙΕΧΟΜΕΝΟ = :content, ΗΜΕΡΟΜΗΝΙΑ = :datee where ID_ΑΝΑΚΟΙΝΩΣΗΣ=$k"; //update των στοιχείων στη βάση
