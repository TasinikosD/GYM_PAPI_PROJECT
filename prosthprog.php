...
<!-- φόρμα για προσθήκη νέου προγράμματος με επιλογή είδους ,ημέρας,ώρας και γυμναστή σύμφωνα με τα δεδομένα της βάσης -->
    ...
    <div class="video-overlay header-text">
        <div class="caption">
            <form action="" method="POST">
                ...
                <p class="forma">Ώρα: <input type="time" name="ora" required></p><br>
                <p class="forma">Γυμναστής: <select type="text" name="gymn" required>
                    <option></option>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
                ...
                // Υπολογισμός λήξης βάσει της διάρκειας
                ...
                //Έλεγχος για κόντρα ωρών με υπάρχοντα προγράμματα
                $result5 = []; 
                ...
                $stmt = $conn->prepare("SELECT ΩΡΑ FROM προγραμμα WHERE ΗΜΕΡΟΜΗΝΙΑ = :hmera");
                ...
                // Έλεγχος για κοινές ώρες προγραμμάτων
                if (!empty($result5)) {
                    foreach ($result5 as $row5) {
                        ...
                        // Αν υπάρχει κοινή ώρα με άλλο πρόγραμμα, απαγορεύουμε την προσθήκη
                        if (($ora_timestamp >= $start_time && $ora_timestamp < $xronos) || 
                            ...
                            $xrhsh = 0; // Υπάρχει κοινή ώρα
                            break;
                        } else {
                            $xrhsh = 1; // Δεν υπάρχει κοινή ώρα
                        }
                    }
                }
                ...
                if ($xrhsh == 1) {
                    try {
                        // Εισαγωγή νέου προγράμματος στη βάση
                        $sql = "INSERT INTO προγραμμα (ΕΙΔΟΣ, ΗΜΕΡΟΜΗΝΙΑ, ΩΡΑ, ΓΥΜΝΑΣΤΗΣ_ID_ΓΥΜΝΑΣΤΗ, ΜΕΓΙΣΤΟΣ_ΑΡΙΘΜΟΣ_ΟΜΑΔΑΣ) VALUES (:eidos, :hmera, :ora, :gymn, :arat)";
                        ...
                        $stmt->execute();
                        echo "<script>window.location.href = 'http://localhost/GYM/orologio.php';</script>";//αλλαγή σελίδας
                        exit();
